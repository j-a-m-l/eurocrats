describe Eurocrat::Vies::Validation do
  let(:valid_a) { 'IE6388047V' }
  let(:valid_b) { 'LU21416127' }
  let(:invalid_a) { 'LU121' }
  let(:invalid_b) { 'IE112' }
  let(:vn_a) { Eurocrat::VatNumber.new valid_a }
  let(:vn_b) { Eurocrat::VatNumber.new valid_b }
    ...
    context 'receiving VAT numbers (strings)' do
      ...
      context 'if they are valid' do
        it 'converts them to VatNumber objects and establish them as the VAT numbers' do
          ...
          expect(subject.requester_vat_number).to be_a Eurocrat::VatNumber
          expect(subject.vat_number).to be_a Eurocrat::VatNumber
  ...
  describe '#request' do
    ...
    let(:stub_response!) {
      stub = expect(Eurocrat::Vies).to receive(:validate_vat_number)
